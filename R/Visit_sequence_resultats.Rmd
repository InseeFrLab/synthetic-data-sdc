...
df[, "depress"] <- factor(df[, "depress"], levels = 0:21)
...
tictoc::tic()
syn_classique <- syn(df)
tictoc::toc()
# 41.857 sec elapsed

pMSE_classique <- utility.gen(syn_classique, df)$pMSE
# 0.01945928
```
...
tictoc::tic()
syn_numfac1 <- syn(df, visit.sequence = vs_numfac1)
tictoc::toc()
# 45.447 sec elapsed

pMSE_numfac1 <- utility.gen(syn_numfac1, df)$pMSE
# 0.02106936
```
...
pMSE_num <- utility.gen(syn_num, df[, num])$pMSE
# 0.004352581
...
pMSE_fac <- utility.gen(syn_fac, df[, fac])$pMSE
# 0.007969088
```
...
tictoc::tic()
syn_nummoda <- syn(df[, num], visit.sequence = vs_nummoda)
tictoc::toc()
# 0.738 sec elapsed

pMSE_nummoda <- utility.gen(syn_nummoda, df[, num])$pMSE
# 0.005557061

# fac : sex, agegr, placesize, edu, socprof, marital, ls, depress, trust, trustfam, trustneigh, sport, smoke, alcabuse alcsol, wkabint, englang
vs_facmoda <- c(14,15,1,13,12,17,9,10,11,16,4,2,6,3,7,5,8)
tictoc::tic()
syn_facmoda <- syn(df[, fac], visit.sequence = vs_facmoda)
tictoc::toc()
# 4.648 sec elapsed

pMSE_facmoda <- utility.gen(syn_facmoda, df[, fac])$pMSE
# 0.01569002
```
...
tictoc::tic()
syn_numfac2 <- syn(df, visit.sequence = vs_numfac2)
tictoc::toc()
# 8.211 sec elapsed

pMSE_numfac2 <- utility.gen(syn_numfac2, df)$pMSE
# 0.02092508
```

```{r AFDM}
df.afdm <- FAMD(df, ncp = 100, graph = FALSE)
summary(df.afdm)

eig.val <- get_eigenvalue(df.afdm)

classement_var <- function() {
  matrice <- matrix(0, nrow = 1, ncol = length(df))
  for (i in 1:length(df)) {
    var <- 0
    for (j in 1:eig.val[, "variance.percent"]) {
      var <- var + eig.val[, "variance.percent"][j] * df.afdm$var$contrib[names(df)[i], j]
    }
    var <- var / 100
    matrice[1, i] <- var
  }
  colnames(matrice) <- names(df)
...
tictoc::tic()
syn_afdm <- syn(df, visit.sequence = vs_afdm)
tictoc::toc()
# 54.214 sec elapsed

pMSE_afdm <- utility.gen(syn_afdm, df)$pMSE
# 0.01787327
```
...
tictoc::tic()
syn_aleatoire <- syn(df, visit.sequence = vs_aleatoire)
tictoc::toc()
# 71.526 sec elapsed

pMSE_aleatoire <- utility.gen(syn_aleatoire, df)$pMSE
# 0.02272717
```
...
tictoc::tic()
syn_noncorrele <- syn(df, visit.sequence = vs_noncorrele) 
tictoc::toc()
# 60.558 sec elapsed

pMSE_noncorrele <- utility.gen(syn_noncorrele, df)$pMSE
# 0.01784704
```
